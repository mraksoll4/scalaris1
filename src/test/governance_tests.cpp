...
BOOST_FIXTURE_TEST_CASE(governance_tests_proposals, TestChainPoS)
{
    ...
    // Check proposal copy constructor
    {
        gov::Proposal proposal1("Test proposal-1", nextSuperblock(chainActive.Height(), consensus.superblock), 3000*COIN,
                         EncodeDestination(dest), "https://forum.scalaris.info", "Short description");
    ...
    gov::Proposal p2a("Test proposal 2", nextSuperblock(chainActive.Height(), consensus.superblock), 3000*COIN,
            EncodeDestination(dest), "https://forum.scalaris.info", "");
    ...
    gov::Proposal p2n("Test proposal max", nextSuperblock(chainActive.Height(), consensus.superblock), 3000*COIN,
                     EncodeDestination(dest), "https://forum.scalaris.info", p2m.getDescription() + "1");
    ...
    gov::Proposal p3("Test proposal-3", nextSuperblock(chainActive.Height(), consensus.superblock), 3000*COIN,
                     EncodeDestination(dest), "https://forum.scalaris.info", "This is a long description that causes the proposal to fail. Proposals are limited by the OP_RETURN size");
    ...
    gov::Proposal p5("Test proposal", nextSuperblock(chainActive.Height(), consensus.superblock), 3000*COIN,
                     "fjdskjfksdafjksdajfkdsajfkasjdfk", "https://forum.scalaris.info", "Short description");
...
BOOST_FIXTURE_TEST_CASE(governance_tests_undo_submissions, TestChainPoS)
{
    ...
    for (int i = 0; i < 25; ++i) {
        ...
        gov::Proposal prop("Test Proposal Undo 2", nextSuperblock(chainActive.Height(), consensus.superblock), 1500*COIN,
                EncodeDestination(dest), "https://forum.scalaris.info", "2");
...
BOOST_AUTO_TEST_CASE(governance_tests_superblockresults)
{
    ...
    // Check superblock proposal and votes
    {
        ...
        // Test single superblock
        {
            ...
            for (int i = 0; i < 16; ++i) {
                const gov::Proposal proposal{strprintf("Test Proposal A%d", i), gov::NextSuperblock(consensus), 250*COIN, saddr, "https://forum.scalaris.info", "Short description"};
        ...
        // Next superblock batch
        {
            ...
            std::set<gov::Vote> votes;
            for (int i = 0; i < 5; ++i) {
                const gov::Proposal proposal{strprintf("Test Proposal B%d", i), gov::NextSuperblock(consensus), 250*COIN, saddr, "https://forum.scalaris.info", "Short description"};
                proposals.insert(proposal);
                CTransactionRef tx;
                auto success = gov::SubmitProposal(proposal, {pos.wallet}, consensus, tx, g_connman.get(), &failReason);
                BOOST_REQUIRE_MESSAGE(success, strprintf("Proposal submission failed: %s", failReason));
        ...
        const gov::Proposal voteCutoffProposal{"Test Vote Cutoff", gov::NextSuperblock(consensus), 250*COIN, saddr, "https://forum.scalaris.info", "Short description"};
        ...
        // Test proposal cutoff
        {
            ...
            const gov::Proposal proposal{"Test Proposal Cutoff", gov::NextSuperblock(consensus), 250*COIN, saddr, "https://forum.scalaris.info", "Short description"};
            CTransactionRef tx;
            auto success = gov::SubmitProposal(proposal, {pos.wallet}, consensus, tx, g_connman.get(), &failReason);
            BOOST_REQUIRE_MESSAGE(success, strprintf("Proposal submission failed: %s", failReason));
            pos.StakeBlocks(1), SyncWithValidationInterfaceQueue();
...
BOOST_AUTO_TEST_CASE(governance_tests_rpc)
{
    ...
    // Check createproposal rpc
    {
        ...
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal $", nextSB, 250, saddr, "https://forum.scalaris.info", "Short description" });
            BOOST_CHECK_THROW(CallRPC2("createproposal", rpcparams), std::runtime_error);
        }
        // Fail on bad superblock
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", nextSB + 1, 250, saddr, "https://forum.scalaris.info", "Short description" });
            BOOST_CHECK_THROW(CallRPC2("createproposal", rpcparams), std::runtime_error);
        }
        // Fail on bad amount (too small)
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", nextSB, consensus.proposalMinAmount - 1, saddr, "https://forum.scalaris.info", "Short description" });
            BOOST_CHECK_THROW(CallRPC2("createproposal", rpcparams), std::runtime_error);
        }
        // Fail on bad amount (too large)
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", nextSB, consensus.proposalMaxAmount + 1, saddr, "https://forum.scalaris.info", "Short description" });
            BOOST_CHECK_THROW(CallRPC2("createproposal", rpcparams), std::runtime_error);
        }
        // Fail on bad address
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", nextSB, 250, "bvkbvkbvkbvkbkvkbkvkbkbvkvkkbkvk", "https://forum.scalaris.info", "Short description" });
            BOOST_CHECK_THROW(CallRPC2("createproposal", rpcparams), std::runtime_error);
        }
        ...
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", nextSB, 250, saddr, "https://forum.scalaris.info" });
            BOOST_CHECK_NO_THROW(CallRPC2("createproposal", rpcparams));
        }
        ...
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", 0, 250, saddr, "https://forum.scalaris.info", "Short description" });
            BOOST_CHECK_NO_THROW(CallRPC2("createproposal", rpcparams));
        }
        // Fail on string amount
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", nextSB, "250", saddr, "https://forum.scalaris.info", "Short description" });
            BOOST_CHECK_THROW(CallRPC2("createproposal", rpcparams), std::runtime_error);
        }
        // Fail on string superblock
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", strprintf("%d", nextSB), 250, saddr, "https://forum.scalaris.info", "Short description" });
            BOOST_CHECK_THROW(CallRPC2("createproposal", rpcparams), std::runtime_error);
        }
        // Fail on long description
        {
            UniValue rpcparams(UniValue::VARR);
            rpcparams.push_backV({"Test proposal 1", nextSB, 250, saddr, "https://forum.scalaris.info", "Long description Long description Long description Long description Long description Long "
    ...
    // Check vote rpc
    {
        ...
        const gov::Proposal proposal{"Test proposal 2", nextSB, 250*COIN, saddr, "https://forum.scalaris.info", "Short description"};
        CTransactionRef tx;
        std::string failReason;
        auto success = gov::SubmitProposal(proposal, {pos.wallet}, consensus, tx, g_connman.get(), &failReason);
        BOOST_REQUIRE_MESSAGE(success, strprintf("Proposal submission failed: %s", failReason));
    ...
    // Check listproposals rpc
    {
        ...
        const gov::Proposal proposal{"Test proposal 3", nextSB, 250*COIN, saddr, "https://forum.scalaris.info", "Short description"};
        CTransactionRef tx;
        auto success = gov::SubmitProposal(proposal, {pos.wallet}, consensus, tx, g_connman.get(), &failReason);
        BOOST_REQUIRE_MESSAGE(success, strprintf("Proposal submission failed: %s", failReason));
        pos.StakeBlocks(1), SyncWithValidationInterfaceQueue();
